...
const Footer = () => {
  ...
  const footerLinks = {
    product: [
      ...
      { name: "Meal Plans", path: "/meal-suggestion" },
      { name: "Health Goals", path: "/health-goals" },
      { name: "Progress Tracking", path: "/health-progress" },
    ],
    company: [
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-2">
              <Apple size={32} />
              <span className="text-2xl font-bold">NutriMeal</span>
            </div>
          ...
          {Object.entries(footerLinks).map(([category, links]) => (
            <div key={category} className="space-y-4">
              <h3 className="footer-title uppercase">{category}</h3>
              <ul className="space-y-2">
                {links.map((link) => (
                  <li key={link.name}>
                    <Link 
                      to={link.path}
                      className="hover:text-primary transition-colors duration-200 flex items-center gap-1 text-sm opacity-90 hover:opacity-100"
                    >
                      {link.name}
      ...
      <div className="border-t border-neutral-content/10">
        <div className="max-w-7xl mx-auto px-4 py-4">
          <div className="flex flex-col md:flex-row justify-between items-center gap-4 text-sm opacity-70">
            <p>© {currentYear} NutriMeal. All rights reserved.</p>
            <div className="flex gap-4">
